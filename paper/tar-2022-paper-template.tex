...
\abstract{
Irony, by itself, is complicated.
And detecting irony in tweets is a difficult task.
In this paper, we analyse tokens extracted from ironic tweets and find which are considered most ironic.
To choose which tokens to analyse, we looked at attention values produced by the RoBERTa model when classifying tweets.
The selected tokens were then appended to selected tweets from train and test sets from the SemEval2018 Task3 dataset.
From the results, by comparing the negative change in the non ironic sentiment, we conclude that the most influental token and token pair are "great" and "Success \textvisiblespace " respectively.
}
...
A sample of the top 3 and bottom 3 changes in non-ironic confidence scores in the train can be seen in Table~\ref{tab:token-single-train-results}, with the full results available at: (LINK TO GITHUB).
...
The full test set results can be seen here: (LINK NA GITHUB).
...
A sample of the top and bottom 3 changes in non-ironic confidence scores in the train set can be seen in Table~\ref{tab:token-pair-train-results}, with the full results available at: (LINK TO GITHUB).
...
Results have shown that the most influental token and token pair are "great" and "Success \textvisiblespace ".
...
Even though the most influential token pair seems to be meaningless when appended to tweets, it is interesting to see tokens like "wow" and "great wow" have great influence on the ironic sentiment.
